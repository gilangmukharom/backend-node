    ...
    const resp = await AgreementProducts.findOne({
      ...
      include: [
        ...
        },
        {
          model: Users,
          as: "renter",
          attributes: ["id", "uuid", "url", "name", "email", "nohp"],
        },
      ],
      where: {
        [Op.or]: [
...
export const createAgreementProducts = async (req, res) => {
  const { time, amount, time_unit, total_price, status, productId, ownerId } =
    req.body;
  try {
...
export const deleteAgreementProducts = async (req, res) => {
  ...
  if (agreementProduct.ownerId !== req.userId && req.role !== "admin") {
